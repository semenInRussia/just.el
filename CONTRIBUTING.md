...
- Verify Emacs Lisp code with `eldev lint`
...
- Use a `just-` prefix for all public names.
- Use a `just--` prefix for all internal names.
...
- Consider using [Flycheck](https://github.com/flycheck/flycheck) to automate `checkdoc` while you're editing.
...
- Use `dash` functions instead of `mapc` and `mapcar` from the standard library
